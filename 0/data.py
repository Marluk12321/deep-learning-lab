...
def graph_data(X, Y_, Y):
    ...
    for i in range(C):
        ...
        color = str(colors[i])
        plt.scatter(X[tp,0], X[tp,1], c=color, marker="o")
        plt.scatter(X[fn,0], X[fn,1], c=color, marker="s")
...
def eval_perf_multi(Y, Y_):
    ...
    precision = diagonal / np.sum(M, axis=0)#np.zeros((C,))
    recall = diagonal / np.sum(M, axis=1)#np.zeros((C,))
